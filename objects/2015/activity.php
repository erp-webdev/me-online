	...
	if ($logged == 1 && $profile_dbname != 'ECINEMA' && $profile_dbname != 'EPARKVIEW' && $profile_dbname != 'NEWTOWN' && $profile_dbname != 'LAFUERZA') {
        ...
        if ($_POST['btnregsub'] || $_POST['btnregsub_x']) :
            ...
            if ($slot_remain > 0) :
                ...
                $_POST['registry_pickup_location'] = $_POST['pickup'];

                // UPLOAD VAX CERT
                $image = $_FILES['registry_vaxcert']['tmp_name'];
                $filename = $_FILES['registry_vaxcert']['name'];
                $filesize = $_FILES['registry_vaxcert']['size'];
                $filetype = $_FILES['registry_vaxcert']['type'];
                ...
                //if (($filesize < 2097152) && in_array($extension, $allowedExts)) :
                    ...
                    $path = "uploads/activity/vaxcert/";
                    $newfilename = $profile_dbname . 'vaxcert'.$_POST['registry_uid'].uniqid().'.'.$extension;
                    $target_path = $path.$newfilename; 

                    $filemove = move_uploaded_file($image, $target_path);
                    $_POST['registry_vaxpath'] = $newfilename;
                    $_POST['registry_vaxlastdate'] = date('U', strtotime($_POST['registry_vaxlastdate']));
            ...
            if (($filesize < 10485760) && in_array($extension, $allowedExts)) :
                ...
                if($filemove) :
                    ...
                    if($add_act) : 
                        ...
                        echo '{"success": false, "details": "failed add"}';
                        exit();
                    endif;
                else :
                    echo '{"success": false, "details": "failed upload"}';
                    exit();
                endif;
        ...
        if ($_POST['btnresendact'] || $_POST['btnresendact_x']) :
            ...
            if($_POST['activity_title']) : 
                ...
                foreach ($receivers as $key => $value) :
                    ...
                    $message = "<div style='display: block; border: 5px solid #024485; padding: 10px; font-size: 12px; font-family: Verdana; width: 100%;'><span style='font-size: 18px; color: #024485; font-weight: bold;'>New ME Online Activity Published</span><br><br>Hi ".$value['FullName'].",<br><br>";
