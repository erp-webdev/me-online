...
<p>
    This is to certify that <strong><?php 
        ...
        if($emp_info[0]["DateResigned"]) echo 'was employed'; else echo 'is currently employed'; ?> with <strong><?php 
        echo $emp_info[0]["CompanyName"]; ?></strong> as <strong><?php 
        echo $emp_info[0]["PositionDesc"]." - ".$emp_info[0]["DeptDesc"]."</strong>  Since <strong>".$emp_info[0]["HireDate"]; ?> </strong> <?php 
        if($emp_info[0]["DateResigned"]) echo 'to ' .$emp_info[0]["DateResigned"]; else echo 'up to the present.'; ?>
</p>
<p>
