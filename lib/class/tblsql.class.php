...
class tblsql {
    ...
    public function get_employee_with_wfhclearance($start = 0, $limit = 0, $search = NULL, $count = 0, $dbname = NULL)
    {
        $sql = "SELECT [outer].* FROM ( ";
        $sql .= " SELECT  ROW_NUMBER() OVER(ORDER BY LName ASC) as ROW_NUMBER, ";
        $sql .= " EmpID, FName, MName, LName, FullName, DBNAME, DeptDesc
    ...
    function get_users_birthday($start = 0, $limit = 0, $bday = NULL, $count = 0, $not = 0)
	{
        ...
        $sql .= " AND EmailAdd IS NOT NULL AND BirthDate IS NOT NULL  and DBNAME <> 'MARKETING' ";
        $sql .= ") AS [outer] ";
        if ($limit) :
            $sql .= " WHERE [outer].[ROW_NUMBER] BETWEEN ".(intval($start) + 1)." AND ".intval($start + $limit)." ORDER BY [outer].[ROW_NUMBER] ";
        endif;
    ...
    function get_registration($id = 0, $start = 0, $limit = 0, $count = 0, $uid = NULL, $db = NULL)
	{
		...
		$sql .= " r.registry_id, a.activity_id, a.activity_title, a.activity_type, a.activity_venue, a.activity_datestart, a.activity_dateend, a.activity_backout, r.registry_uid, r.registry_godirectly, r.registry_vrin, r.registry_vrout, r.registry_details, r.registry_platenum, r.registry_child, r.registry_guest, r.registry_dependent, r.registry_date, r.registry_status, r.registry_hash, r.registry_vaxpath, registry_vaxstatus, registry_vaxlastdate, registry_seat, registry_location, registry_pickup_location ";
		$sql.=" FROM HREventRegistry r 
        LEFT JOIN HRActivity a on r.registry_activityid = a.activity_id ";
    ...
    function get_registrantmd5($id = NULL, $start = 0, $limit = 0, $count = 0, $actid = 0)
	{
		...
		$sql.=" FROM HREventRegistry r
            LEFT JOIN VIEWHREMPMASTER e ON r.registry_uid = e.EmpID ";
		$sql.=" WHERE r.registry_status >= 1
            AND e.CompanyActive = 1 ";
    ...
    function get_staff($start = 0, $limit = 0, $count = 0, $headid = 0)
    {
		...
		$sql.=" FROM VIEWHREMPMASTER e
        LEFT JOIN HREventRegistry r ON  r.registry_uid = e.EmpID";
		$sql.=" WHERE e.Active = 1 AND r.registry_status = 1 AND r.registry_offsite = 0  AND e.CompanyActive = 1 ";
    ...
    function get_approval($type, $ref)
    {
        $sql = "SELECT top 1 * 
            FROM TED_VIEW_NOTIFICATION2 WHERE Reference='" . $ref . "' AND DocType = '" .$type. "'";
			...
			case 'add':
                ...
                foreach ($value as $key => $value) :
                    if (in_array($key, $accepted_field)) :
                        ...
                        elseif ($key == 'registry_activityid' || $key == 'registry_date' || $key == 'registry_dateattend' || $key == 'registry_vaxlastdate') :
