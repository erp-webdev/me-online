...
class mainsql
{
    ...
    function get_employee($start = 0, $limit = 0, $search = NULL, $count = 0)
    {
        ...
        $sql .= " EmpID, FName, MName, LName, NickName, EmailAdd, CompanyID,
            SSSNbr, PhilHealthNbr, TINNbr, PagibigNbr, TaxID, LocationID, AccountNo, EPassword, DB_NAME() as DBNAME
            FROM viewHREmpMaster ";
        $sql .= " WHERE EmpID != '' ";
        if ($search != NULL) : $sql .= " AND (EmpID = '" . $search . "' OR LName LIKE '%" . $search . "%' OR FName LIKE '%" . $search . "%') ";
        endif;
    ...
    function get_approver_employee($start = 0, $limit = 0, $search = NULL, $count = 0, $signatory = NULL, $signatorydb = NULL)
    {
        ...
        if ($search != NULL) : $sql .= " AND (a.EmpID = '" . $search . "' OR LName LIKE '%" . $search . "%' OR FName LIKE '%" . $search . "%') ";
        endif;
    ...
    function get_recent_noti($empid = NULL)
    {
        ...
        if ($empid != NULL) : $sql .= " AND (EmpID = '" . $empid . "' OR Signatory01 = '" . $empid . "' OR Signatory02 = '" . $empid . "' OR Signatory03 = '" . $empid . "' OR Signatory04 = '" . $empid . "' OR Signatory05 = '" . $empid . "' OR Signatory06 = '" . $empid . "') ";
        endif;
    ...
    function get_posi_data($id = 0, $dbname = NULL)
    {
        ...
        if ($id !== 0) : $sql .= " WHERE PositionID = '" . $id . "'";
        endif;
    ...
    function get_dept_data($id = NULL, $dbname = NULL)
    {
        ...
        if ($id != NULL) : $sql .= " WHERE DeptID = '" . $id . "'";
        endif;
    ...
    function get_image($type = 0, $id = 0)
    {
        ...
        if ($type == 2) :
            ...
            if ($id != 0) : $sql .= " WHERE EmpID = '" . $id . "'";
            endif;
        elseif ($type == 3) :
            $sql = "SELECT * FROM HRActivity ";
            if ($id != 0) : $sql .= " WHERE activity_id = '" . $id . "'";
            endif;
        else :
    ...
    function set_emailblock($empid, $value = 0, $new = 0, $dbname = NULL)
    {
        ...
        else : $sql .= " SET ApprovedOrDisapprove = " . $value . " ";
        endif;
    ...
    function get_notification($id = NULL, $start = 0, $limit = 0, $search = NULL, $count = 0, $empid = NULL, $from = NULL, $to = NULL, $dtype = NULL, $dbname = NULL)
    {
        ...
        Remarks01, Remarks02, Remarks03, Remarks04, Remarks05, Remarks06, IsCancellable
        FROM TED_VIEW_NOTIFICATION ";
        $sql .= " WHERE EmpID IS NOT NULL ";
        if ($id != NULL) : $sql .= " AND Reference = '" . $id . "' ";
        endif;
        if ($search != NULL) : $sql .= " AND (Reference LIKE '%" . $search . "%' OR EmpID = '" . $search . "' OR FULLNAME LIKE '%" . $search . "%') ";
        endif;
        if ($dtype != NULL) : $sql .= " AND DocType = '" . $dtype . "' ";
        endif;
        if ($empid != NULL) : $sql .= " AND ((EmpID = '" . $empid . "') OR (Signatory01 = '" . $empid . "' OR Signatory02 = '" . $empid . "' OR Signatory03 = '" . $empid . "' OR Signatory04 = '" . $empid . "' OR Signatory05 = '" . $empid . "' OR Signatory06 = '" . $empid . "') AND Approved != 3) ";
        endif;
        if ($from && $to) :
    ...
    function get_pendingnoti($id = NULL, $start = 0, $limit = 0, $search = NULL, $count = 0, $empid = NULL, $from = NULL, $to = NULL, $dtype = NULL, $dbname = NULL)
    {
        ...
        if ($search != NULL) : $sql .= " AND (Reference LIKE '%" . $search . "%' OR EmpID LIKE '%" . $search . "%' OR FullName LIKE '%" . $search . "%') ";
        endif;
        if ($dtype != NULL) : $sql .= " AND DocType = '" . $dtype . "' ";
        endif;
        if ($empid != NULL) : $sql .= " AND ((EmpID = '" . $empid . "' AND DBNAME = '" . $dbname . "') OR (Signatory01 = '" . $empid . "' OR Signatory02 = '" . $empid . "' OR Signatory03 = '" . $empid . "' OR Signatory04 = '" . $empid . "' OR Signatory05 = '" . $empid . "' OR Signatory06 = '" . $empid . "') AND Approved != 3) ";
        endif;
        if ($from && $to) :
            $sql .= " AND DateFiled BETWEEN '" . $from . " 00:00:00.000' AND '" . $to . " 23:59:59.000' ";
        endif;
    ...
    function get_nrequest($type = 1, $id = NULL, $grp = 0, $start = 0, $limit = 0, $search = NULL, $count = 0, $empid = NULL, $status = 0, $from = NULL, $to = NULL)
    {
        ...
        switch ($type) {
            ...
            case 2: // LEAVE
                ...
                $sql .= " WHERE SeqId != 0 ";
                if ($id != NULL) : $sql .= " AND LeaveRef = '" . $id . "'";
                endif;
                if ($search != NULL) : $sql .= " AND LeaveRef LIKE '%" . $search . "%' ";
                endif;
                if ($empid != NULL) : $sql .= " AND EmpID = '" . $empid . "' ";
                endif;
                if ($status != 0) : $sql .= " AND Approved = '" . $status . "' ";
            ...
            case 5: // CTS
                ...
                $sql .= " WHERE SeqID != 0 ";
                if ($id != NULL) : $sql .= " AND ChangeTimeRef = '" . $id . "'";
                endif;
                if ($search != NULL) : $sql .= " AND ReqNbr LIKE '%" . $search . "%' ";
                endif;
                if ($empid != NULL) : $sql .= " AND EmpID = '" . $empid . "' ";
            ...
            case 6: // NPA
                ...
                $sql .= " WHERE SeqID != 0 ";
                if ($id != NULL) : $sql .= " AND NonPunchRef = '" . $id . "'";
                endif;
                if ($search != NULL) : $sql .= " AND NonPunchRef LIKE '%" . $search . "%' ";
                endif;
                if ($empid != NULL) : $sql .= " AND EmpID = '" . $empid . "' ";
                endif;
            ...
            case 11:
                ...
                $sql .= " WHERE SeqId != 0 ";
                if ($id != NULL) : $sql .= " AND RefNbr = '" . $id . "'";
                endif;
                if ($search != NULL) : $sql .= " AND RefNbr LIKE '%" . $search . "%' ";
                endif;
                if ($empid != NULL) : $sql .= " AND EmpID = '" . $empid . "' ";
                endif;
                if ($from && $to) :
    ...
    function get_attachments($empid = NULL, $count = 0, $dbname = NULL)
    {
        ...
        $sql .= " WHERE ReqNbr != '' ";
        if ($empid != NULL) : $sql .= " AND ReqNbr = '" . $empid . "' ";
        endif;
    ...
    function get_leave($desc = NULL)
    {
        ...
        if ($desc != NULL) : $sql .= " AND LeaveDesc = '" . $desc . "' ";
        endif;
        $result = $this->get_row($sql);
        return $result;
    }
    ...
    function get_leavebytype($empid = NULL, $type = NULL, $year = 0, $count = 0)
    {
        ...
        if ($type != NULL) : $sql .= " AND LeaveDesc = '" . $type . "' ";
        endif;
        if ($empid != NULL) : $sql .= " AND EmpID = '" . $empid . "' ";
        endif;
        if ($year != 0) : $sql .= " AND YEAR(AbsenceFromDate) = " . $year . " ";
        endif;
        $sql .= " ORDER BY DateFiled DESC ";
        if ($count) : $result = $this->get_numrow($sql);
        else : $result = $this->get_row($sql);
    ...
    function get_leaveledgerbytype($empid = NULL, $type = NULL, $year = 0, $count = 0)
    {
        ...
        $sql .= " WHERE SeqId != 0 ";
        if ($type != NULL) : $sql .= " AND LeaveID = '" . $type . "' ";
        endif;
        if ($empid != NULL) : $sql .= " AND EmpID = '" . $empid . "' ";
        endif;
        if ($year != 0) : $sql .= " AND PRYear = " . $year . " ";
        endif;
    ...
    function get_empclearance($empid, $dbname)
    {
        $sql = "SELECT * FROM HREmpClearance 
            WHERE cast(getdate() as date) > cast(created_at as date) 
            AND EmployeeUpdatedAt IS NULL 
            AND EmpID='" . $empid . "' 
            AND DBNAME='" . $dbname . "'";
        $result = $this->get_row($sql, 'SUBSIDIARY');
    ...
    function get_memo($id = 0, $start = 0, $limit = 0, $search = NULL, $count = 0, $from = 0, $to = 0, $type = 0)
    {
        ...
        if ($id != 0) : $sql .= " AND MemoID = " . $id;
        endif;
        if ($search != NULL) : $sql .= " AND (MemoName LIKE '%" . $search . "%') ";
        endif;
        if ($type != 0) : $sql .= " AND MemoType = " . $type;
        endif;
        if ($from && $to) :
    ...
    function get_mrequest($type = 1, $id = NULL, $start = 0, $limit = 0, $search = NULL, $count = 0, $empid = NULL, $status = NULL, $from = NULL, $to = NULL)
    {
        ...
        switch ($type) {
            ...
            case 5: // CTS
                ...
                if ($search != NULL) : $sql .= " AND ChangeTimeRef LIKE '%" . $search . "%' ";
                endif;
                if ($empid != NULL) : $sql .= " AND EmpID = '" . $empid . "' ";
                endif;
                if ($status != 0) : $sql .= " AND Approved = '" . $status . "' ";
            ...
            case 10: // WFH
                ...
                if ($search != NULL) : $sql .= " AND Reference LIKE '%" . $search . "%' ";
                endif;
                if ($empid != NULL) : $sql .= " AND EmpID = '" . $empid . "' ";
                endif;
                if ($status != NULL) : $sql .= " AND Approved = '" . $status . "' ";
    ...
    function get_leavedata($leaveref = NULL)
    {
        ...
        if ($leaveref != NULL) : $sql .= " WHERE LeaveRef = '" . $leaveref . "' AND Status != 'CANCELLED' ";
        endif;
        $result = $this->get_row($sql);
        return $result;
    }
    ...
    function get_mdtrdata($mdtrref = NULL)
    {
        ...
        if ($mdtrref != NULL) : $sql .= " WHERE ReqNbr = '" . $mdtrref . "' AND Status != 'CANCELLED' ";
        endif;
        $result = $this->get_row($sql);
        return $result;
    }
    ...
    function get_obtdata($obtref = NULL)
    {
        ...
        if ($obtref != NULL) : $sql .= " WHERE ReqNbr = '" . $obtref . "' ";
        endif;
    ...
    function get_tsdata($tsref = NULL)
    {
        ...
        if ($tsref != NULL) : $sql .= " WHERE ReqNbr = '" . $tsref . "' AND Status != 'CANCELLED' ";
        endif;
    ...
    function check_appexpire($dtrdate1, $dtrdate2 = NULL)
    {
        ...
        if ($daynum >= 16) :
            if (date('m') == '3')
                $expdate = strtotime(date("y-2-1", strtotime("-1 month")));
            else
    ...
    function check_appexpire_by_attpostdate($dbname, $dtrdate1, $dtrdate2 = null)
    {

        $date = date('Y-m-d', strtotime($dtrdate1));

        $query = "SELECT MAX(AttPostDate) as AttPostDate FROM 
                    (   SELECT DISTINCT TOP (4) PaymentType, PRYear, PeriodID, CutOff, PRFrom, PRTo, PeriodFrom, PeriodTo, AttPostDate
                        FROM HRCompanyCutOff
                        WHERE PRYear >= '" . date('Y', strtotime($date)) . "' AND CompanyID <> 'LGMI01' 
                        AND PeriodID NOT LIKE 'SP%' AND PaymentType <> 'SPECIAL'
                        AND (PRFrom >= '" . $date . "' OR '" . $date . "' BETWEEN PRFrom AND PRTo)
                        ORDER BY PRFrom ASC
                    ) AS T";

        $result = $this->get_row($query);

        // echo "<!-- ".$dbname . " | ". $query . " | DTR ". $date . " expires on " . $result[0]['AttPostDate']. " -->";

        if (strtotime(date('Y-m-d')) <= strtotime($result[0]['AttPostDate'])) {
            if (!empty($dtrdate2)) {
                return $this->check_appexpire_by_attpostdate($dbname, $dtrdate2);
            } else {
                return 0;
            }
        } else {
    ...
    function get_dtr_period_by_id($year, $company, $period, $all, $dbname = NULL)
    {
        $sql = "SELECT TOP 1 PeriodID, PRYear, PRFrom, PRTo, PeriodFrom, PeriodTo, AttPost 
    ...
    function get_dtr_data($empid, $from, $to, $company, $dbname = NULL)
    {
        $sql = "SELECT DISTINCT 
            ...
            TimeINDate, TimeIN, 
            TimeOutDate, TimeOut, 
            LateHrs, UTHrs, Absent, 
            LEAVETYPE, 
            L01, L02, L03, L04, L05, L10, L12, L14, L15, L16, ML,
            (CASE 
                ...
                WHEN Leave16 <> 0 THEN 'VAWC' 
            END) AS LEAVE_DESC,
            OTHrs01, OTHrs02, OTHrs03, OTHrs04, OTHrs05, OTHrs06, OTHrs07, OTHrs08, OTHrs09, OTHrs10, OTHrs11, OTHrs12, OTHrs13, OTHrs14, OTHrs15, OTHrs16, OTHrs17, OTHrs18, OTHrs19, OTHrs20, OTHrs21, OTHrs22, OTHrs23, OTHrs24, OTHrs25, 
            WorkHrs, RegHrs, 
            OB,
            ApprovedOTHrs, ActualOTHrs, NDHrs, CreatedDate, 'HRDTRH' as ReferenceTable 
            FROM viewHRDTR ";
    ...
    function get_dtr_bydate_final($empid, $date)
    {
        $sql = "SELECT DTRDATE, '' as ShiftDesc, TimeINDate, TimeOutDate, TimeIN, TimeOut, LateHrs, UTHrs, Absent, LEAVETYPE, L01, L02, L03, L04, L05, OTHrs01, OTHrs02, OTHrs03, OTHrs04, OTHrs05, OTHrs06, OTHrs07, OTHrs08, OTHrs09, OTHrs10, OTHrs11, OTHrs12, OTHrs13, OTHrs14, OTHrs15, OTHrs16, OTHrs17, OTHrs18, OTHrs19, OTHrs20, OTHrs21, OTHrs22, OTHrs23, OTHrs24, OTHrs25, WorkHrs, RegHrs, OB, ApprovedOTHrs, NDHrs, '' as LEAVE_DESC FROM viewHRDTR_Final ";
        $sql .= " WHERE EmpID = '" . $empid . "' AND DTRDATE = '" . date("m/d/Y", strtotime($date)) . "' ";
        $result = $this->get_row($sql);
        return $result;
    }
    ...
    function get_shift($shiftid = NULL)
    {
        $sql = "SELECT ShiftID, ShiftDesc, TimeIN, TimeOUT, NUMHrs, BreakIN, BreakOUT, BreakHours
            FROM HRShift ";
        $sql .= " WHERE Active = 1 ";
        if ($shiftid != NULL) : $sql .= " AND ShiftID = '" . $shiftid . "' ";
        endif;
    ...
    function get_appliedwfh($empid, $date, $dbname)
    {
      ...
      WHERE B.EmpID = '" . $empid . "' and A.DTRDate = '" . $date . "' and A.Status <> 'CANCELLED' ORDER BY DTRDate";
        $result = $this->get_row($sql, $dbname);
    ...
    function get_logs($id = NULL, $start = 0, $limit = 0, $search = NULL, $count = 0, $from = NULL, $to = NULL)
    {
        ...
        $sql .= " WHERE EmpID != NULL ";
        if ($id != NULL) : $sql .= " AND EmpID = '" . $id . "' ";
        endif;
        if ($search != NULL) : $sql .= " AND (EmpID LIKE '%" . $search . "%') ";
        endif;
        if ($from && $to) :
    ...
    function dtr_action($value, $action, $id = 0, $db = NULL)
    {
        ...
        if ((int)$dtr_day > 15) {
            ...
            $periodto = strtotime(date('Y-m-t 23:59:59', strtotime($value['dteDTRDate'])));
        } else {
