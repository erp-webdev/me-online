...
$(function() {
	...
	$(".btnviewads-quality-policy").on("click", function() {
        $("#adview_title").html("");
		$(".floatdiv").removeClass("invisible");
		$("#adview").show({
            effect : 'slide',
    ...
    $("#registry_godirectly").change(function() {
        ...
        pickup = $("#pickup option:selected").val();
        $('.pickupfrom').each(function (){
            $(this).html(pickup);
        });
		...
		$.ajax(
	    {
	        ...
	        success: function(data) {
                ...
                if (!obj.activity_venue || obj.activity_venue == "" || obj.activity_venue == null) { 
                    $("#registry_venue").html('The World Center'); 
                } else { 
                    $("#registry_venue").html(obj.activity_venue); 
                }
                ...
                if (obj.registry_godirectly == 1) { 
                    $("#registry_godirectly").html('<i class="fa fa-check fa-lg greentext"></i>'); 
                } else { 
                    $("#registry_godirectly").html('<i class="fa fa-times fa-lg redtext"></i>'); 
                }

                if (obj.activity_venue && obj.activity_venue != "" && obj.activity_venue != null) {

                    if (obj.registry_vrin == 1) { 
                        vrin = '<i class="fa fa-check fa-lg greentext"></i>'; 
                    } else { 
                        vrin = '<i class="fa fa-times fa-lg redtext"></i>'; 
                    }

                    if (obj.registry_vrout == 1) { 
                        vrout = '<i class="fa fa-check fa-lg greentext"></i>'; 
                    } else { 
                        vrout = '<i class="fa fa-times fa-lg redtext"></i>'; 
                    }

                    $("#registry_vr").html('<b>Will ride the company vehicle:</b><br />From ' + obj.registry_pickup_location + ' to ' + obj.activity_venue + ': ' + vrin + '<br />From ' + obj.activity_venue + ' to ' + obj.registry_pickup_location + ': ' + vrout);
                }
                ...
                if (obj.registry_status == 1) { 
                    $("#registry_status").html('<span class="redtext">For approval</span>'); 
                } else { 
                    $("#registry_status").html('<span class="greentext">Approved</span>'); 
                }

                if (obj.registry_status == 4) { 
                    $("#btndelreg2").addClass("invisible"); 
                } else { 
                    $("#btndelreg2").removeClass("invisible"); 
                }
    ...
    $("#searchwciman").on("keypress", function(e) {
        if (e.keyCode == 13) {

            searchwciman = $("#searchwciman").val();

            wcimanpage = 1;

            $.ajax(
            {
                url: "<?php echo WEB; ?>/lib/requests/wciman_request.php?sec=table",
                data: "searchwciman=" + searchwciman,
                type: "POST",
                complete: function(){
                    $("#loading").hide();
                },
                success: function(data) {
                    $("#btnwcimanall").removeClass("invisible");
                    $("#wcimandata").html(data);
                    changeUrl('', '<?php echo WEB; ?>/wcimanan');
                }
            })
        }
    });

    $("#btnwciman").on("click", function() {
    ...
    $("#btnwcimanall").on("click", function() {
        ...
        $.ajax(
        {
            ...
            success: function(data) {
                $("#searchwciman").val("");
                $("#btnwcimanall").addClass("invisible");
                $("#wcimandata").html(data);
                changeUrl('', '<?php echo WEB; ?>/wcimanan');
            }
        })
