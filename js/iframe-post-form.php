...
$(function ()
{
	...
	({
		...
		complete : function (response)
		{
			...
			{
				html += '<p>We just sent you the reset link, kindly check your inbox. The password reset link is valid for an hour only. </p>';
    ...
    ({
        ...
        post : function ()
        {
            ...
            if (!$('.rpass_msg').length)
            {
                $('#rpass_title').after('<div class="rpass_msg" style="display:none; padding:10px; margin-top:10px; margin-bottom:10px; text-align:center" />');
            }

            if ($('#resetpassword').val().length)
            {
                $('.rpass_msg')
                .html('<i class="fa fa-refresh fa-spin fa-lg"></i> Processing&hellip;')
                .css({
 ...
 *                                   */
    ...
    ({
        ...
        post : function ()
        {
            ...
            {
				$('.paf_msg')
                    .html('"Promotion To Level:" must not be blank ')
                    .css({
                        color : '#9c0006',
                        background : '#ffc7ce',
                        border : '2px solid #9c0006',
            ...
            if($('input[name=promotion]').val() != ''){
                var currentrank_ = $('#currentrank').val().trim();
                var promoteto = $('input[name=promotion]').val().trim();
                var newposition = $('input[name=promotetoPos]').val()
                if(currentrank_ != promoteto && promoteto != 'NOT FOR PROMOTION' && newposition == ''){
                    $('.paf_msg')
                        .html('New Position Title is required when promoted to a new level.')
                        .css({
                            color : '#9c0006',
                            background : '#ffc7ce',
                            border : '2px solid #9c0006',
